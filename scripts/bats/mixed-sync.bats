...
@test "Add Different Elements To Nodes & Check For Successfull Sync" {
  ports="$(docker ps | awk '/set/ {print $1}' | xargs -I {} docker port {} 8080 | sed ':a;N;$!ba;s/\n/,/g' | sort)"
	...
	response="$(curl -sS -i http://$node1/set/add --data '{"value": "6"}' | awk ' /HTTP/ {print $2}')" && [ "$response" == "200" ]

	response="$(curl -sS -i http://$node2/set/add --data '{"value": "1"}' | awk ' /HTTP/ {print $2}')" && [ "$response" == "200" ]
	response="$(curl -sS -i http://$node2/set/add --data '{"value": "2"}' | awk ' /HTTP/ {print $2}')" && [ "$response" == "200" ]
	response="$(curl -sS -i http://$node2/set/add --data '{"value": "3"}' | awk ' /HTTP/ {print $2}')" && [ "$response" == "200" ]
	...
	# response="$(curl -sS -X GET http://$node1/set/list)" && echo $response && [ "$response" == "[1,2,6,3,4,5]" ]
	# response="$(curl -sS -X GET http://$node2/set/list)" && echo $response && [ "$response" == "[1,2,6,3,4,5]" ]
